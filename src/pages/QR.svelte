<script lang="ts">
  ...
  const handleText = () => {
    ...
    schemas.text
      .validate($textForm)
      .then(async () => {
        input.set(await QRCode.toDataURL($textForm.text));
      })
      .catch((err) => {
  ...
  const handleURL = () => {
    ...
    schemas.url
      .validate($urlForm)
      .then(async () => {
        input.set(await QRCode.toDataURL($urlForm.url));
      })
      .catch((err) => {
        for (let e of err.errors) {
          addToast({ message: e.toString() });
  ...
  const handlePhone = () => {
    schemas.phone
      .validate($phoneForm)
      .then(async () => {
        input.set(await QRCode.toDataURL(`tel:${$phoneForm.number}`));
      })
      .catch((err) => {
        for (let e of err.errors) {
          addToast({ message: e.toString() });
  ...
  const handleWifi = () => {
    ...
    schemas.wifi
      ...
      .then(async () => {
        ...
        input.set(await QRCode.toDataURL(wifiStr));
      })
      .catch((err) => {
        console.log(err);
        for (let e of err.errors) {
  ...
  const handleOtp = () => {
    ...
    schemas.otp
      ...
      .then(async () => {
        ...
        if ($otpForm.type == "totp") {
          //  make period when otp.type is totp
        ...
        input.set(await QRCode.toDataURL(final));
...
<div id="content">
  {#if active == "Text"}
    <div class={`p-4 rounded-lg bg-gray-800`}>
      <section>
        <label for="text1" class="block mb-2 text-sm font-medium text-white"
          >Enter text</label
        >
        <input
          type="text"
        ...
        {#if $otpForm.type == "hotp"}
          <div class="mb-6">
            ...
            <input
              ...
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder={Math.floor(Math.random() * 100000).toString()}
