...
<p class="mb-3 text-xs text-gray-400 text-left">
  Shortened URLs last for 24 hours.
</p>
<form on:submit|preventDefault={handleSubmit}>
  <div class="flex">
    <div class="relative w-full">
      ...
      <div class="mt-3">
        <HCaptcha
          sitekey="9f24247e-be32-4234-a548-c8aedbc48320"
          bind:this={captcha}
          on:success={(e) => {
            captchaToken = e.detail.token;
          }}
          on:error={() => {
            addToast("Captcha failure");
            captcha.reset();
          }}
        />
      </div>
...
{#if res}
  ...
  {:then data}
    <Card title="Shortened URL" description={data.short}>
      ...
      <button
        use:copy={data.short}
        class="inline-flex items-center text-blue-600 bg-gray-100 hover:bg-gray-300"
      >
        Copy
        <svg
          class="w-5 h-5 ml-2"
  ...
  {:catch err}
    <Card
      title={err.response?.data?.error ?? err.name}
